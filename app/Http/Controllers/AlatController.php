...
class AlatController extends Controller
{
    ...
    public function destroy($id) {
     ...
     // Fungsi untuk mendapatkan rekomendasi alat outdoor berdasarkan peringkat pengguna
     public function rekomenAlat(Request $request, $alatId)
    {
        $userId = $request->user()->id ?? null; // Ambil ID pengguna dari input request
        $currentUserRatings = Rating::where('user_id', $userId)->get();

        // Ambil semua peringkat pengguna lain, kecuali pengguna saat ini
        $otherUserRatings = Rating::where('user_id', '!=', $userId)->get();

        // Kumpulkan informasi peringkat berdasarkan alat_id
        $alatRatings = [];
        foreach ($otherUserRatings as $rating) {
            if (!isset($alatRatings[$rating->alat_id])) {
                $alatRatings[$rating->alat_id] = [];
            }
            $alatRatings[$rating->alat_id][$rating->user_id] = $rating->rating;
        }

        // Hitung kesamaan antara pengguna saat ini dan pengguna lain menggunakan metode Cosine Similarity
        $similarities = [];
        foreach ($alatRatings as $alatId => $ratings) {
            $currentUserRatingsForAlat = $currentUserRatings->where('alat_id', $alatId)->first();

            if ($currentUserRatingsForAlat) {
                $currentUserRating = $currentUserRatingsForAlat->rating;
                $numerator = 0;
                $denominator1 = 0;
                $denominator2 = 0;

                foreach ($ratings as $userRating) {
                    $numerator += $currentUserRating * $userRating;
                    $denominator1 += pow($currentUserRating, 2);
                    $denominator2 += pow($userRating, 2);
                }

                $similarity = $numerator / (sqrt($denominator1) * sqrt($denominator2));
                $similarities[$alatId] = $similarity;
            }
        }

        // Urutkan kesamaan dalam urutan menurun
        arsort($similarities);

        // Ambil alat yang belum dinilai oleh pengguna saat ini
        $recommendedAlat = Alat::whereNotIn('id', $currentUserRatings->pluck('alat_id')->toArray())
            ->orderBy('created_at', 'desc')
            ->get();

        // Ambil 5 alat teratas berdasarkan kesamaan
        $topRecommendedAlat = [];
        $count = 0;
        foreach ($recommendedAlat as $alat) {
            if (isset($similarities[$alat->id])) {
                $topRecommendedAlat[] = $alat;
                $count++;
            }
        ...
        return view('member.rekomen', ['recommendedAlat' => $topRecommendedAlat,]);
    }
}
