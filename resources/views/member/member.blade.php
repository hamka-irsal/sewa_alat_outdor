                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-footer">
                                <form action="{{ route('cart.store',['id' => $item->id, 'userId' => Auth::user()->id]) }}" method="POST">
                                    ...
                                    {{-- <a class="link-dark" href="{{ route('member.rate_alat') }}"> --}}                                    
                                </form>
                                <a class="nav-link {{ Route::is('member.rate_alat') ? 'active': '' }}" href="{{ route('member.rate_alat') }}">Rate</span></a>

            

                                {{-- @if ($userRating = $item->where('id', $item->id)->first())
                                <span class="badge badge-primary">Peringkat: {{ $userRating->rating }}</span>
                                @endif
                                <form action="/rate/{{ Auth::user()->id }}/{{ $item->id }}/" method="POST" style="display: inline;">
                                @csrf
